...
for k in range(len(filenames2a)):
    ...
    print('Annual arctic water vapor feedback: ' + np.str(q_feedback[k]) + ' W m^-2 K^-1')
    print('Monthly arctic water vapor feedback: ' + np.str(np.round(q_feedback_mon[k], 3)) + ' W m^-2 K^-1')

outfile = open('Arctic_Water_Vapour_Feedback_1955', 'wb')
pickle.dump(q_feedback, outfile)
outfile.close()

outfile = open('Arctic_Water_Vapour_Feedback_Monthly_1955', 'wb')
pickle.dump(q_feedback_mon, outfile)
outfile.close()

outfile = open('Arctic_Water_Vapour_Feedback_Flux_1955', 'wb')
pickle.dump(q_feedback_flux, outfile)
outfile.close()

outfile = open('Arctic_Water_Vapour_Feedback_Flux_Monthly_1955', 'wb')
pickle.dump(q_feedback_flux_mon, outfile)
outfile.close()

np.savetxt('Arctic_Water_Vapour_Feedback_1955.csv', q_feedback, delimiter = ',')
np.savetxt('Arctic_Water_Vapour_Feedback_Monthly_1955.csv', q_feedback_mon, delimiter = ',')

np.savetxt('Arctic_Water_Vapour_Feedback_Flux_1955.csv', q_feedback_flux, delimiter = ',')
np.savetxt('Arctic_Water_Vapour_Feedback_Flux_Monthly_1955.csv', q_feedback_flux_mon, delimiter = ',')
