...
for k in range(len(data1)):
    ...
    dts_globalmean_mon2 = (np.nansum(np.nansum(np.multiply(dts, weight_mon),axis=1),axis=0))

    for i in range(len(dts_globalmean_mon2)):
        dts_globalmean_mon2[i] = dts_globalmean_mon2[i]/np.nansum(weight)

    dts_globalmean_mon.append(dts_globalmean_mon2)
        

outfile = open('dts_arctic_monthly_1955', 'wb')
pickle.dump(dts_globalmean_mon, outfile)
outfile.close()

np.savetxt('dts_arctic_monthly_1955.csv', dts_globalmean_mon, delimiter = ',')
