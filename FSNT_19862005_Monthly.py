...
for k in range(len(data)):
    ...
    for i in range(np.shape(FSNT[0])[2]):
        for j in range(np.shape(FSNT[0])[1]):
            for m in range(12):
                temp.append(np.nanmean(FSNT[k][792+m:1032+m:12,j,i]))
            temp2.append(temp)
            temp = []
        FSNT_19812005.append(temp2)
        temp2 = []
    
    outfile = open('FSNT_19862005_monthly' + np.str(k + 1), 'wb')
    pickle.dump(FSNT_19812005, outfile)
    outfile.close()
    FSNT_19812005 = []
