...
for k in range(len(data)):
    ...
    for i in range(np.shape(SHFLX[0])[2]):
        for j in range(np.shape(SHFLX[0])[1]):
            for m in range(12):
                if k == 0:
                    temp.append(np.nanmean(SHFLX[k][1632+m:1872+m:12,j,i]))
                else:
                    temp.append(np.nanmean(SHFLX[k][792+m:1032+m:12,j,i]))
            temp2.append(temp)
            temp = []
        SHFLX_19812005.append(temp2)
        temp2 = []
    
    if k == 0:
        outfile = open('SHFLX_19862005_monthly1', 'wb')
        pickle.dump(SHFLX_19812005, outfile)
        outfile.close()
        SHFLX_19812005 = []
        
    else:
        outfile = open('SHFLX_19862005_monthly' + np.str(k + 1), 'wb')
        pickle.dump(SHFLX_19812005, outfile)
        outfile.close()
        SHFLX_19812005 = []
