...
for k in range(len(data1)):
    ...
    dts_globalmean_mon.append(np.nansum(np.nansum(np.multiply(dts, weight_mon),axis=1),axis=0))


outfile = open('dts_globalmean_monthly_1955', 'wb')
pickle.dump(dts_globalmean_mon, outfile)
outfile.close()

np.savetxt('dts_globalmean_monthly_1955.csv', dts_globalmean_mon, delimiter = ',')
