...
for k in range(len(data1)):
    ...
    print(['Annual Tropics temperature feedback: ' + np.str(t_feedback[k]) + ' W m^-2 K^-1'])
    print(['Monthly Tropics temperature feedback: ' + np.str(np.round(t_feedback_mon[k], 3)) + ' W m^-2 K^-1'])
    
outfile = open('Tropics_Temperature_Feedback_1955', 'wb')
pickle.dump(t_feedback, outfile)
outfile.close()

outfile = open('Tropics_Temperature_Feedback_Monthly_1955', 'wb')
pickle.dump(t_feedback_mon, outfile)
outfile.close()

outfile = open('Tropics_Temperature_Feedback_Flux_1955', 'wb')
pickle.dump(t_feedback_Flux, outfile)
outfile.close()

outfile = open('Tropics_Temperature_Feedback_Flux_Monthly_1955', 'wb')
pickle.dump(t_feedback_Flux_mon, outfile)
outfile.close()

np.savetxt('Tropics_Temperature_Feedback_1955.csv', t_feedback, delimiter = ',')
np.savetxt('Tropics_Temperature_Feedback_Monthly_1955.csv', t_feedback_mon, delimiter = ',')

np.savetxt('Tropics_Temperature_Feedback_Flux_1955.csv', t_feedback_Flux, delimiter = ',')
np.savetxt('Tropics_Temperature_Feedback_Flux_Monthly_1955.csv', t_feedback_Flux_mon, delimiter = ',')
