...
for k in range(len(data1)):
    ...
    dts_globalmean.append(np.nansum(np.nansum(np.multiply(np.nanmean(dts, axis=2), weight),axis=1),axis=0))
    print(dts_globalmean[k])

outfile = open('dts_globalmean_annual_1955', 'wb')
pickle.dump(dts_globalmean, outfile)
outfile.close()

np.savetxt('dts_globalmean_annual_1955.csv', dts_globalmean, delimiter = ',')
