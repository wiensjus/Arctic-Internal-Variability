...
for k in range(len(filenames1a)):
    ...
    dSW_alb_globalmean = np.nansum(np.nansum(np.nanmean(dSW_alb, 2) * weight, 1), 0)/np.nansum(weight)
    dSW_alb_globalmean_mon = np.nansum(np.nansum((dSW_alb * weight_mon), 1), 0)
    
    for i in range(len(dSW_alb_globalmean_mon)):
        dSW_alb_globalmean_mon[i] = dSW_alb_globalmean_mon[i]/np.nansum(weight)
    ...
    print(['Annual arctic surface albedo feedback: ' + np.str(alb_feedback[k]) + ' W m^-2 K^-1'])
    print(['Monthly arctic surface albedo feedback: ' + np.str(np.round(alb_feedback_mon[k], 3)) + ' W m^-2 K^-1'])

    
outfile = open('Arctic_Albedo_Feedback_1955', 'wb')
pickle.dump(alb_feedback, outfile)
outfile.close()

outfile = open('Arctic_Albedo_Feedback_Monthly_1955', 'wb')
pickle.dump(alb_feedback_mon, outfile)
outfile.close()

outfile = open('Arctic_Albedo_Feedback_Flux_1955', 'wb')
pickle.dump(alb_feedback_flux, outfile)
outfile.close()

outfile = open('Arctic_Albedo_Feedback_Flux_Monthly_1955', 'wb')
pickle.dump(alb_feedback_flux_mon, outfile)
outfile.close()

np.savetxt('Arctic_Albedo_Feedback_1955.csv', alb_feedback, delimiter = ',')
np.savetxt('Arctic_Albedo_Feedback_Monthly_1955.csv', alb_feedback_mon, delimiter = ',')

np.savetxt('Arctic_Albedo_Feedback_Flux_1955.csv', alb_feedback_flux, delimiter = ',')
np.savetxt('Arctic_Albedo_Feedback_Flux_Monthly_1955.csv', alb_feedback_flux_mon, delimiter = ',')
