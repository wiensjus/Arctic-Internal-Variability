...
for k in range(len(data)):
        ...
        for j in range(np.shape(FSDS[0])[1]):
            for m in range(12):
                if k == (len(filenames) - 1):
                    temp.append(np.mean(FSDS[k][1632+m:1872+m:12,j,i]) - np.mean(FSDS[k][1260+m:1500+m:12,j,i]))
                else:
                    temp.append(np.mean(FSDS[k][792+m:1032+m:12,j,i]) - np.mean(FSDS[k][420+m:660+m:12,j,i]))
            temp2.append(temp)
            temp = []
    ...
    if k == (len(filenames) - 1):
        outfile = open('dFSDS_monthly1_1955', 'wb')
        pickle.dump(FSDS_19812005, outfile)
        outfile.close()
        FSDS_19812005 = []
        
    else:
        outfile = open('dFSDS_monthly' + np.str(k + 2) + '_1955', 'wb')
        pickle.dump(FSDS_19812005, outfile)
        outfile.close()
        FSDS_19812005 = []
