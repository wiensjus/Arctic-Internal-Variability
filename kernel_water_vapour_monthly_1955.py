...
for k in range(len(filenames2a)):
    ...
    # between LW and SW!
    dR_logq_globalmean = np.nansum(np.nansum(np.nanmean(-dLW_logq + dSW_logq, 2) * weight, 1), 0)
    ...
    print('Annual water vapor feedback: ' + np.str(q_feedback[k]) + ' W m^-2 K^-1')
    print('Monthly water vapor feedback: ' + np.str(np.round(q_feedback_mon[k], 3)) + ' W m^-2 K^-1')

outfile = open('Water_Vapour_Feedback_1955', 'wb')
pickle.dump(q_feedback, outfile)
outfile.close()

outfile = open('Water_Vapour_Feedback_Monthly_1955', 'wb')
pickle.dump(q_feedback_mon, outfile)
outfile.close()

outfile = open('Water_Vapour_Feedback_Flux_1955', 'wb')
pickle.dump(q_feedback_flux, outfile)
outfile.close()

outfile = open('Water_Vapour_Feedback_Flux_Monthly_1955', 'wb')
pickle.dump(q_feedback_flux_mon, outfile)
outfile.close()

np.savetxt('Water_Vapour_Feedback_1955_Monthly.csv', q_feedback, delimiter = ',')
np.savetxt('Water_Vapour_Feedback_Monthly_1955.csv', q_feedback_mon, delimiter = ',')

np.savetxt('Water_Vapour_Feedback_Flux_1955.csv', q_feedback_flux, delimiter = ',')
np.savetxt('Water_Vapour_Feedback_Flux_Monthly_1955.csv', q_feedback_flux_mon, delimiter = ',')
