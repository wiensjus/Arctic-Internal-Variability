...
for k in range(len(filenames1a)):
    ...
    print(['Annual Tropics surface albedo feedback: ' + np.str(alb_feedback[k]) + ' W m^-2 K^-1'])
    print(['Monthly Tropics surface albedo feedback: ' + np.str(np.round(alb_feedback_mon[k], 3)) + ' W m^-2 K^-1'])

    
outfile = open('Tropics_Albedo_Feedback_1955', 'wb')
pickle.dump(alb_feedback, outfile)
outfile.close()

outfile = open('Tropics_Albedo_Feedback_Monthly_1955', 'wb')
pickle.dump(alb_feedback_mon, outfile)
outfile.close()

outfile = open('Tropics_Albedo_Feedback_Flux_1955', 'wb')
pickle.dump(alb_feedback_flux, outfile)
outfile.close()

outfile = open('Tropics_Albedo_Feedback_Flux_Monthly_1955', 'wb')
pickle.dump(alb_feedback_flux_mon, outfile)
outfile.close()

np.savetxt('Tropics_Albedo_Feedback_1955.csv', alb_feedback, delimiter = ',')
np.savetxt('Tropics_Albedo_Feedback_Monthly_1955.csv', alb_feedback_mon, delimiter = ',')

np.savetxt('Tropics_Albedo_Feedback_Flux_1955.csv', alb_feedback_flux, delimiter = ',')
np.savetxt('Tropics_Albedo_Feedback_Flux_Monthly_1955.csv', alb_feedback_flux_mon, delimiter = ',')
